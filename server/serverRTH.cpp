...
int main() {
    // Loading booking file. 
    try {
        bookings = readBookingsFromFile("RTH.txt");
    ...
    ssize_t msglen;
    udp_sockfd = create_udp_socket(UDP_PORT_SERVER_RTH); // Call the utility function to create and bind a UDP socket
    cout << "The Server RTH is up and running using UDP on port " << UDP_PORT_SERVER_RTH << "\n";
    ...
    std::string message = "Hello, from serverRTH ";
    std::string message_type = "BOOT";
    std::string sender = "ClientRTH";
    
    send_message_over_udp(sockfd, message_type, sender, message);
    std::cout << "The Server RTH has informed the main server." << endl;
...
void* listener(void* arg) {
    ...
    while(1) {
        ...
        if (msg.messageType == "AVAIL_REQ") {
            cout << "\nThe Server RTH received an availability request from the main server." << endl;
            std::string response;
            ...
            std::string sender = "serverRTH";
            send_message_over_udp(sockfd, message_type, sender, msg.content);
            std::cout << "\nThe Server RTH finished sending the response to the main server." << endl;
            terminate_socket(sockfd);
        } else if (msg.messageType == "RESER_REQ") {
            cout << "\nThe Server RTH received an reservation request from the main server." << endl;
            std::string response;
